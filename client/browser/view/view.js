    ...
    class View extends SimpleEvent {
      constructor(options) {
        ...
        if (this.events) {
          ...
          for (k in ref) {
            v = ref[k];
            m = k.match(/^(\S+)\s*(.*)$/);
            this.$el.on(`${m[1]}.delegateEvents${this._id}`, m[2].replace('&-', `${this.className}-`), typeof v !== 'string' ? _.bind(v, this) : ((v) => {
              return () => {
                return this[v]();
              };
            })(v));
      ...
      render() {
        ...
        this.$el.find('[class]').forEach((el) => {
          return $(el).attr('class', $(el).attr('class').replace('&-', `${this.className}-`));
        });
